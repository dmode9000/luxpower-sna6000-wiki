...
Як і у випадку з напругою, досягнення цього відсотка **не зупиняє розряд акумулятора**. Інвертор продовжить живити підключені побутові прилади, поки заряд не впаде до критичного порогу відключення (`Off-grid Cut-off SOC` або `On-grid EOD SOC`). Цей параметр слугує виключно як завчасне сповіщення для користувача (щоб вимкнути зайві навантаження) та як системна команда для автоматичного запуску генератора.
...
- **Мінімум:** Дорівнює встановленому вами порогу повного відключення батареї (`Cut-off SOC`).
- **Максимум:** 90%.
- **Крок:** 1%.
- **За замовчуванням:** 20%.
...
- Зазвичай встановлюється на **5% – 10% вище** за ваш поріг повного відключення (`Cut-off SOC`). Наприклад, якщо інвертор вимикається при 15%, логічно встановити попередження на 20% або 25%. Це дасть вам певний запас часу (і ємності) для реакції.

## Примітки та важливі особливості

> [!NOTE] **Залежність від типу керування розрядом:**
> Цей параметр активний лише тоді, коли розряд керується за відсотком заряду (`Batt Discharge Control` встановлено на `SOC`). Тобто, це налаштування працює для літієвих батарей з підключеним кабелем комунікації (BMS). Якщо комунікації немає, інвертор орієнтуватиметься на сусідній параметр у Вольтах.

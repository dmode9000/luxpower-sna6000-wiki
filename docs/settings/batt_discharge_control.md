...
> Зверніть увагу, що ваш вибір у цьому меню автоматично змінить формат наступних критично важливих налаштувань: порогів відключення `On-grid EOD` (розряд за наявності мережі) та `Off-grid Cut-off` (розряд під час блекауту).
>
...
> Це налаштування має логічно відповідати обраному типу батареї. Якщо у `Battery Type` ви обрали `Lithium` і налаштували протокол зв'язку, тут варто виставити `SOC` (але в окремих випадках можна використовувати і `Voltage`). Якщо ви обрали `Lead-acid` — система очікуватиме роботу за `Voltage`.

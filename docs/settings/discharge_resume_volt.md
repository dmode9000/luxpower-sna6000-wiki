...
**Приклад роботи:**
Якщо ваш поріг відключення без мережі (`Off-Grid Cut-Off Volt`) встановлено на **47.0 В**, а буфер `Discharge Resume Volt` встановлено на **2.0 В**:
...
> цей параметр є глобальним гістерезисом. Він застосовується одразу до кількох налаштувань: до порогу відключення без мережі (`Off-grid Cut-off Volt`), до порогу переходу на мережу (`On-grid Cut-off Volt`), а також до порогу відновлення після попередження про низький заряд (`Battery low warning point`).

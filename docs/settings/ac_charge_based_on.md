...
- **`Battery SOC (According to)`:** Зарядка за відсотком заряду (SOC). Aктивується, коли рівень заряду падає нижче [`Start AC Charge SOC`](start_ac_charge_soc), і зупиняється при досягненні [`Stop AC Charge SOC`](stop_ac_charge_soc)
...
> Якщо під час активного `AC Charge` часовий проміжок [`AC First`](ac_first_mode) буде неактивним, то при управлінні по SOC чи напрузі при досягненні кінцевого рівня SOC чи напруги, інвертор почне розряджати батарею поки вона не досягне початкового рівня SOC чи напруги для заряду від мережі. Якщо в період AC Charge ви не бажаєте щоб батарея починала розряджатись після заряду, активуйте такий самий часовий проміжок [`AC First`](ac_first_mode).

> [!NOTE] Залежні параметри
> Вибір режиму в цьому випадному списку — це лише активація логіки. Щоб зарядка фактично запрацювала, вам потрібно задати відповідні пороги старту/зупинки (`Start AC Charge SOC` та `Stop AC Charge SOC`), встановити хочаб один з часових проміжків ([`Time 1, Time 2, Time 3`](ac_charge_t1_t2_t3)), якщо обрано режим із часом, а також вказати максимальний струм заряду від мережі у параметрі [`AC Charge Battery Current`](ac_charge_battery_current).

> [!WARNING] Перемикач EPS
> Якщо апаратний перемикач **EPS** (знаходиться знизу інвертора) вимкнений — AC Charge **не працюватиме**, навіть якщо всі програмні налаштування виконано правильно. Це часта причина, чому зарядка від мережі не стартує попри коректні параметри. Перевірте положення перемикача EPS перед діагностикою.

...
1. **Мережа присутня (On-Grid)**: Тут логіка залежить від налаштування `PV&AC Take Load Jointly`:
   - 👉 **Якщо `PV&AC Take Load Jointly` УВІМКНЕНО**: Ви обов'язково повинні увімкнути функцію `Export to Grid`. Тільки тоді порт AC Couple запуститься
   ...
   - 👉 **Якщо `PV&AC Take Load Jointly` ВИМКНЕНО (Базовий режим)**: Порт AC Couple запуститься навіть без дозволу на експорт в мережу. Енергія піде на навантаження резервної лінії (EPS) та заряджання батареї, а всі надлишки будуть автоматично "зрізатися" за допомогою функції зсуву частоти (Frequency Shifting)

1. **Мережа відсутня (Off-Grid / Блекаут):**
...
> Щоб мережевий інвертор не вимикався, а постійно підтримував генерацію під час блекауту (регулюючи її зсувом частоти), можна встановити параметр AC Couple Start SOC на рівні 101%, або Start Volt — на недосяжно високе значення.
